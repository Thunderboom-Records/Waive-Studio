...
<div class="flex flex-col gap-2 p-2 mx-1 w-28 items-start justify-start">
	...
	<button
		...
		class="bg-{instrument.color}-500 hover:bg-{instrument.color}-600 btn btn-sm rounded-full w-12 h-6 font-medium text-xs capitalize leading-8"
	>
		new
	</button>
	<button
		on:click={() => dispatch("newClip", {variation: true})}
		class="{variation ? `bg-${instrument.color}-500 hover:bg-${instrument.color}-600` : 'bg-gray-600'} font-medium leading-8 capitalize btn btn-sm rounded-full w-12 h-6 text-xs
